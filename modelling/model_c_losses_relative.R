...
# global model
m_global_losses_perc <- lmer(log(losses_perc) ~
...
# check the model
# vif(m_global_losses_perc)
check_collinearity(m_global_losses_perc)

func_mult_diag(m_global_losses_perc, data_mod$long, data_mod$lat, k_n)

# model selection
# run all possible models including year as predictor and n_max predictors
models_ranked_losses_perc <- dredge(m_global_losses_perc,
                                 m.lim = c(0, n_max),
                                 fixed =
...
selected_models_losses_perc <-
  (subset(models_ranked_losses_perc, subset = delta <= 4))
selected_models_losses_perc

# average the selected models
averaged_model_losses_perc <- summary(model.avg(selected_models_losses_perc))
averaged_model_losses_perc

model_result_table(averaged_model_losses_perc, "losses_perc")

# R2 of the best model
best_model_losses_perc <- get.models(selected_models_losses_perc, subset = 1)[[1]] # best model
r.squaredGLMM(best_model_losses_perc)

check_collinearity(best_model_losses_perc)

#################################################

# save.image(file = "models_losses_perc.RData")
