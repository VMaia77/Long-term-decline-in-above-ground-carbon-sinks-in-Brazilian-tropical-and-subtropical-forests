...
# global model
m_global_c_stocks <- lmer(log(c_stocks_ha) ~
...
# check the model
# vif(m_global_c_stocks)
check_collinearity(m_global_c_stocks)

func_mult_diag(m_global_c_stocks, data_mod$long, data_mod$lat, k_n)

# model selection
# run all possible models including year as predictor and n_max predictors
models_ranked_c_stocks <- dredge(m_global_c_stocks,
  m.lim = c(0, n_max),
  fixed =
...
selected_models_c_stocks <-
  (subset(models_ranked_c_stocks, subset = delta <= 4))
selected_models_c_stocks

# average the selected models
averaged_model_c_stocks <- summary(model.avg(selected_models_c_stocks))
averaged_model_c_stocks

model_result_table(averaged_model_c_stocks, "c_stocks")

# R2 of the best model
best_model_c_stocks <- get.models(selected_models_c_stocks, subset = 1)[[1]] # best model
r.squaredGLMM(best_model_c_stocks)

check_collinearity(best_model_c_stocks)

#################################################

# save.image(file = "models_c_stocks.RData")
